...
namespace Vinesense.Batch.Migrators
{
    class Site2Migrator : SiteMigrator<Site2>
    {
        ...
        public override void MigrateLogs(DbContext context, Site2 site)
        {
            ...
            MigrateLog(context, timestamp, (float)site.S4Moist, 5, SensorType.Moisture);
            MigrateLog(context, timestamp, (float)site.S4Temp, 5, SensorType.Temperature);
            MigrateLog(context, timestamp, (float)site.S5Moist, 7, SensorType.Moisture);
            MigrateLog(context, timestamp, (float)site.S5Temp, 7, SensorType.Temperature);
        }
    }
}
